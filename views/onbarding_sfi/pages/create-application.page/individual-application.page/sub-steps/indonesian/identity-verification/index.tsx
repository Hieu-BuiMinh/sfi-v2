...
function IndoIdentityVerificationFormStep() {
	...
	const methods = useForm<IdentityVerificationFormData>({
		...
		defaultValues: {
			verification_document:
				currentIndiApp?.content?.customer_particular?.identify_verification?.verification_document || 'ktp',
			ktp_or_passport:
			...
			npwp_number: currentIndiApp?.content?.customer_particular?.identify_verification?.npwp_number || '',
			npwp_photo: { file: null, previewUrl: '', base64: '' },
		},
	})

	const onSubmit = (data: IdentityVerificationFormData) => {
		if (currentIndiApp) {
			const { front, selfie, npwp_photo, ...identify_verification } = data
						...
						content: {
							...
							customer_particular: {
								...
								identify_verification: {
									...
									...(npwp_photo?.previewUrl && {
										npwp_photo: npwp_photo.previewUrl.replace(`${apiBase}/storage/`, ''),
									}),
								},
							},
						},
...
export default IndoIdentityVerificationFormStep
