...
function AdminWithdrawalGridView() {
	const t = useTranslations('admin.transactions.list')
	const [params, setParams] = useAdminTransactionTableParams('withdrawal')

	const { data: response, isLoading } = useQuery({
		queryKey: adminFinanceTransactionService.getWithdrawalTransactions.key({
			page: params.page,
		...
		queryFn: () =>
			adminFinanceTransactionService.getWithdrawalTransactions.get({
				page: params.page,
				per_page: params.per_page,
				transaction_id: params.search || undefined,
				status: params.status || undefined,
	...
	const columns: GridColDef<TTransaction>[] = [
		...
		},
		{
			...
			renderCell: (params) => (
				<Link
					href={`/transactions/withdrawal/${params.value}`}
		...
		},
		{
			...
			width: 180,
			valueGetter: (_, row) => row.sfi_withdraw_approval?.user_approve?.name || '-',
		},
		{
		...
		{
			field: 'amount',
			headerName: t('table.columns.withdrawal_amount'),
			width: 150,
			renderCell: (params) => (
...
export default AdminWithdrawalGridView
