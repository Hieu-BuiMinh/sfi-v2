...
import { resetStaffListTableParams, useStaffListTableParams } from '../hooks/use-staff-list-table-params'
...
function AdminStaffTab() {
	...
	const [params, setParams] = useStaffListTableParams()
	...
	const selectedDept = useMemo(() => {
		...
		return departmentOptions.find((opt: any) => opt.value === params.department) || null
	}, [params.department, departmentOptions])

	const selectedLocation = useMemo(() => {
		if (!params.location) return null
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		return locationOptions.find((opt: any) => opt.value === params.location) || null
	...
	const columns: GridColDef<TUserListItem>[] = [
						...
						<BaseDropdownMenu
							...
							items={[
								...
								},
								{
									...
									onClick: () => {
										router.push(`/settings/authority/staff/${params.row.id}/log`)
									},
								},
							]}
						/>
		...
		},
		{
			...
			renderCell: (params) => (
				<Link
					...
					className="text-mui-primary cursor-pointer underline"
				>
					{params.row.email || '-'}
				</Link>
			),
		},
		{
			...
			width: 200,
			renderCell: (params) => (
				<span className="text-mui-text-primary font-semibold">{params.row.name || '-'}</span>
			),
		},
		{
			field: 'position',
			...
			width: 150,
			valueGetter: (_, row) => row.positions?.map((p) => p.name).join(', ') || '-',
		},
		{
			field: 'department',
		...
		},
		{
			...
			width: 150,
			valueGetter: (_, row) => row.sale_codes?.map((s) => s.code).join(', ') || '-',
		},
		// {
		//   field: 'referral_link',
		...
		<div className="flex flex-col gap-6">
			<div className="flex flex-wrap items-center gap-4">
				...
				<Button
					...
					onClick={() => resetStaffListTableParams(setParams)}
					size="medium"
				>
