...
import { getStaffFormSchema, IStaffFormValues, StaffFormProps } from './staff-form.types'
...
export function StaffForm({ onSuccess, onCancel }: StaffFormProps) {
	...
	const { mutate, isPending } = useMutation({
		...
		onError: (error: any) => {
			toast.error(error?.response?.data?.message || t('messages.create_staff_error'))
		},
	})

	const onSubmit = (data: IStaffFormValues) => {
		const selectedPosition = positionsResponse?.data?.find((p) => p.id === data.position)
		...
		const payload: TCreateStaffRequest = {
			...
			profile: {
				gender: data.gender,
				date_of_birth: data.date_of_birth ? dayjs(data.date_of_birth).unix() : 0,
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col">
			<div className="max-h-[70vh] flex-1 space-y-6 overflow-x-hidden overflow-y-auto p-1">
				<div className="grid grid-cols-1 gap-x-12 gap-y-6 px-1 md:grid-cols-2">
					{/* Left Column */}
						...
						<RfhSfiSingleAutocomplete
							...
							placeholder={
								selectedLocationId
									...
									: t('form.add_staff.fields.department.placeholder_select_location')
							}
							onChange={() => {
								setValue('manager', undefined)
							}}
			...
			<div className="border-mui-divider mt-8 flex justify-end gap-3 border-t pt-6">
				<Button onClick={onCancel} variant="outlined" className="min-w-30">
