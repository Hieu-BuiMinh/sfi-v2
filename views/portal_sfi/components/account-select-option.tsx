...
export interface AccountSelectOptionProps {
	account: AccountItemByType
	type: 'LIVE' | 'TRIAL'
}

export default function AccountSelectOption({ account, type }: AccountSelectOptionProps) {
	return (
		<div className="flex items-center gap-3">
			<div className="bg-mui-bg-subtle text-mui-text-secondary flex h-8 w-8 shrink-0 items-center justify-center rounded uppercase">
				<UserAvt />
			</div>
			<div className="flex flex-col gap-0.5">
				<div className="text-mui-text-primary text-sm font-semibold">
					{account.BindingAccount && account.BindingAccount + '-'} {account.Name}
				</div>
				<div className="text-mui-text-secondary flex items-center gap-2 text-xs">
					<span className={type === 'LIVE' ? 'text-success-main font-medium' : 'text-mui-text-secondary'}>
						{type === 'LIVE' ? 'Live' : 'Demo'}
					</span>
					<Divider orientation="vertical" flexItem sx={{ height: 12, my: 'auto' }} />
					<span>{account.Login}</span>
					<Divider orientation="vertical" flexItem sx={{ height: 12, my: 'auto' }} />
					<span className="text-mui-text-primary font-medium">
						{formatMoney(account.Balance, { currency: 'USD', useSymbol: true })}
					</span>
				</div>
			</div>
		</div>
